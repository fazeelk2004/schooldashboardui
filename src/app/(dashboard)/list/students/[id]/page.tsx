    ...
    <div className="flex-1 p-6 flex flex-col gap-6 xl:flex-row">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="panel panel-pad lg:col-span-2 flex flex-col sm:flex-row gap-5">
            ...
            <div className="flex-1 flex flex-col gap-3 min-w-0">
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-2.5 text-sm mt-2">
                <InfoRow icon="blood" label="Blood" value={student.bloodType || "—"} />
                <InfoRow
                  icon="date"
                  label="Birthday"
          ...
          <div className="grid grid-cols-2 gap-3 lg:col-span-1">
            <StatTile
              ...
              label="Attendance"
              value={
                <Suspense fallback={<span className="text-ink-subtle text-sm">…</span>}>
            ...
            <StatTile
              ...
              label="Lessons"
              value={<span className="text-2xl font-semibold text-ink">{student.class._count.lessons}</span>}
            />
            <StatTile
              icon={
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 21V8a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2v13"/><path d="M3 21h18"/><path d="M9 21V11"/></svg>
              }
              label="Grade"
              value={<span className="text-2xl font-semibold text-ink">{student.class.name.charAt(0)}th</span>}
            />
            <StatTile
              icon={
              ...
              label="Class"
              value={<span className="text-2xl font-semibold text-ink">{student.class.name}</span>}
            />
          </div>
        </div>
      ...
      <div className="w-full xl:w-1/3 flex flex-col gap-6">
        <div className="panel panel-pad">
          ...
          <p className="text-xs text-ink-subtle mt-0.5">Quick links related to this student.</p>
          <div className="mt-4 grid grid-cols-2 gap-2">
            <Shortcut href={`/list/lessons?classId=${student.class.id}`} label="Lessons" />
            <Shortcut href={`/list/teachers?classId=${student.class.id}`} label="Teachers" />
            <Shortcut href={`/list/exams?classId=${student.class.id}`} label="Exams" />
            <Shortcut href={`/list/assignments?classId=${student.class.id}`} label="Assignments" />
            <Shortcut href={`/list/results?studentId=${student.id}`} label="Results" />
          </div>
        </div>
        <Performance />
        <Announcements />
